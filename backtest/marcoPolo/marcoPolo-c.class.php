...
class MarcoPolo extends Fx {
	...
	function __construct($system, $configArr)
	{
		...
		$this->settings = array(
			...
			"USD_CAD" => array(
				...
				"tpMultiplier" => 4,
				"slMultiplier" => 1,
				"windowRatioThresholdBuyMin" => .10,
				"windowRatioThresholdBuyMax" => .25,
				"windowRatioThresholdSellMin" => .75,
				"windowRatioThresholdSellMax" => .90,
	...
	function execute()
	{
		...
		foreach ($this->currencyBasket as $idx=>$pairToTrade) {
			...
			//// if day is Friday and hour is 15, close all trades
			//if (date("D", $this->getTickTime()) == "Fri" && date("H", $this->getTickTime()) == 15 && count($trades->trades) > 0) {
...
//			if (date("D", $this->getTickTime()) == "Mon" && date("H", $this->getTickTime()) == 2 && count($trades->trades) == 0) {
			if (count($trades->trades) == 0) {
				...
				if ($cot['Market_and_Exchange_Names'] != "") {
					...
					//if ($cot['Tot_Rept_Positions_Long_All'] > $cot['Tot_Rept_Positions_Short_All'] && $cot['Change_in_Tot_Rept_Long_All'] > 0 && $cot['Change_in_Tot_Rept_Short_All'] < 0) {
						//$cotBias = "up";
					//} else if  ($cot['Tot_Rept_Positions_Long_All'] < $cot['Tot_Rept_Positions_Short_All'] && $cot['Change_in_Tot_Rept_Short_All'] > 0 && $cot['Change_in_Tot_Rept_Long_All'] < 0) {
						//$cotBias = "down";
					//} else {
						//$cotBias = "none";
					//}
					
					if ($cot['Lev_Money_Positions_Long_All'] > $cot['Lev_Money_Positions_Short_All']) {
						$cotBias = "up";
					} else if  ($cot['Lev_Money_Positions_Long_All'] < $cot['Lev_Money_Positions_Short_All']) {
						$cotBias = "down";
					} else {
						$cotBias = "none";
					}
					
					//if ($cot['Change_in_Tot_Rept_Long_All'] > 0 && $cot['Tot_Rept_Positions_Short_All'] < 0) {
						//$cotBias = "up";
					//} else if  ($cot['Change_in_Tot_Rept_Long_All'] < 0 && $cot['Tot_Rept_Positions_Short_All'] > 0) {
						//$cotBias = "down";
					//} else {
						//$cotBias = "none";
					//}
					
					print "total long=".$cot['Tot_Rept_Positions_Long_All']."\n";
					print "total short=".$cot['Tot_Rept_Positions_Short_All']."\n";
