	...
	function __construct($system, $configArr)
	{
		...
		$this->settings = array(
			...
			"USD_CAD" => array(
				...
				"lookbackHours" => 24*1.5,
				"tpMultiplier" => 2.1,
				"slMultiplier" => 1.75,
				"windowRatioThresholdBuyMin" => 0,
				"windowRatioThresholdBuyMax" => .3,
				"windowRatioThresholdSellMin" => .7,
	...
	function execute()
	{
		...
		foreach ($this->currencyBasket as $idx=>$pairToTrade) {
			...
			if (0 && count($trades->trades) > 0) {
				...
				$totalPositions = $cot['Lev_Money_Positions_Long_All'] + $cot['Lev_Money_Positions_Short_All'];
