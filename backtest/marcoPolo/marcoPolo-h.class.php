...
//			if (date("D", $this->getTickTime()) == "Mon" && date("H", $this->getTickTime()) == 2 && count($trades->trades) == 0) {
			if (count($trades->trades) == 0) {
				...
				if ($cot['Market_and_Exchange_Names'] != "") {
					...
					if ($longPerc > $shortPerc && $longPerc >= $this->settings[$pairToTrade]['entryPercMin'] && $cot['Change_in_Lev_Money_Long_All'] > 0) {
						$cotBias = "up";
					} else if  ($shortPerc > $longPerc && $shortPerc >= $this->settings[$pairToTrade]['entryPercMin'] && $cot['Change_in_Lev_Money_Short_All'] > 0) {
						$cotBias = "down";
					} else {
						$cotBias = "none";
					}
				...
				if ($exp[0] == "USD") {
					...
					// swap the long percent and short perc
					$longPerc = $cot['Lev_Money_Positions_Short_All'] / $totalPositions;
					$shortPerc = $cot['Lev_Money_Positions_Long_All'] / $totalPositions;
					
					// also make the bias variable the reverse of the cotBias variable
					if ($cotBias == "up") { $bias = "down"; }
					else if ($cotBias == "down") { $bias = "up"; }
					else { $bias = "none"; }
				...
				if ($bias != "none") {
					...
					if (count($trades->trades) == 0) {
						...
						if ($bias == "down" && !$this->traded($pairToTrade, $cot['AvailableDate'])) {
							...
							// short
							$actualTpMultiplier = (($shortPerc-$spreadPerc) * ($this->settings[$pairToTrade]['tpMultiplier']-$this->settings[$pairToTrade]['slMultiplier'])) + $this->settings[$pairToTrade]['slMultiplier'];
//							$actualTpMultiplier = $this->settings[$pairToTrade]['tpMultiplier'] - ($this->settings[$pairToTrade]['tpMultiplier']*$spreadPerc);
							
							$TP = $candle->closeAsk - ($range * $actualTpMultiplier);
							...
							// long
							$actualTpMultiplier = (($longPerc-$spreadPerc) * ($this->settings[$pairToTrade]['tpMultiplier']-$this->settings[$pairToTrade]['slMultiplier'])) + $this->settings[$pairToTrade]['slMultiplier'];
//							$actualTpMultiplier = $this->settings[$pairToTrade]['tpMultiplier'] - ($this->settings[$pairToTrade]['tpMultiplier']*$spreadPerc);

							$TP = $candle->closeBid + ($range * $actualTpMultiplier);
