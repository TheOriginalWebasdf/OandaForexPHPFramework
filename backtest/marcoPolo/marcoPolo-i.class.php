	...
	function __construct($system, $configArr)
	{
		...
		$this->settings = array(
			...
			"USD_CAD" => array(
				...
				"entryPercMin" => .55,			// when to enter a trade.  if COT percentage of a given direction >= this percentage, look for a trade
				"slAdjustPercentage" => .50,	// when quote >= this percentage of the way to TP, begin to adjust SL
			...
			"AUD_USD" => array(
				...
				"windowRatioThresholdSellMax" => .9,
				"entryPercMin" => .5,			// when to enter a trade.  if COT percentage of a given direction >= this percentage, look for a trade
				"slAdjustPercentage" => .6,		// when quote >= this percentage of the way to TP, begin to adjust SL
...
//			if (date("D", $this->getTickTime()) == "Mon" && date("H", $this->getTickTime()) == 2 && count($trades->trades) == 0) {
			if (count($trades->trades) == 0) {
				...
				if ($cot['Market_and_Exchange_Names'] != "") {
					...
					if ($longPerc > $shortPerc && 
						$longPerc >= $this->settings[$pairToTrade]['entryPercMin'] && 
						$cot['Change_in_Lev_Money_Long_All'] > 0 &&
						$cot['Change_in_Lev_Money_Long_All'] > $cot['Change_in_Lev_Money_Short_All']) {
							
						$cotBias = "up";
						
					} else if  ($shortPerc > $longPerc && 
								$shortPerc >= $this->settings[$pairToTrade]['entryPercMin'] && 
								$cot['Change_in_Lev_Money_Short_All'] > 0 &&
								$cot['Change_in_Lev_Money_Short_All'] > $cot['Change_in_Lev_Money_Long_All']) {
