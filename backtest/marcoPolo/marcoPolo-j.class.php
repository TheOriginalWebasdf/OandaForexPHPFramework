	...
	function __construct($system, $configArr)
	{
		...
		$this->settings = array(
			...
			"USD_CAD" => array(
				...
				"tpMultiplier" => 3.5,
				"slMultiplier" => 3.25,
				"windowRatioThresholdBuyMin" => .05,
				"windowRatioThresholdBuyMax" => .10,
				"windowRatioThresholdSellMin" => .90,
				"windowRatioThresholdSellMax" => .95,
				"entryPercMin" => .0,			// when to enter a trade.  if COT percentage of a given direction >= this percentage, look for a trade
				"slAdjustPercentage" => .5,	// when quote >= this percentage of the way to TP, begin to adjust SL
				...
				if ($bias != "none") {
					...
					if (count($trades->trades) == 0) {
						...
						if ($bias == "down" && !$this->traded($pairToTrade, $cot['AvailableDate'])) {
							...
							$calcUnits -= $calcUnits * $spreadPerc;
