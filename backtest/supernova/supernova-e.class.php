	...
	function __construct($system, $configArr)
	{
		...
		$this->settings = array(
			...
			"EUR_USD" => array(
				...
				"tradeLookbackCandles" => 6*48,  // 24 hrs
		...
		foreach ($this->currencyBasket as $idx=>$pairToTrade) {
			...
			if (count($trades->trades) == 0) {
				...
				if (count($trades->trades) == 0) {
					...
					if ($windowClosingRatio > $this->settings[$pairToTrade]['windowRatioThresholdSell']) {
...
//						$SL = $candle->closeAsk + ($tradeWindowRange * $this->settings[$pairToTrade]['slMultiplier']);
					...
					} else if ($windowClosingRatio < $this->settings[$pairToTrade]['windowRatioThresholdBuy']) {
