	...
	function __construct($system, $configArr)
	{
		...
		$this->settings = array(
			...
			"EUR_USD" => array(
				...
				"tpMultiplier" => 6,
				"minSL" => .0020,
				"tradeHour" => array(3,4,5,6,7,8,9,10),
			),
	...
	function execute()
	{
		...
		foreach ($this->currencyBasket as $idx=>$pairToTrade) {
			...
			if (count($trades->trades) == 0 && in_array(date("H", $this->getTickTime()), $this->settings[$pairToTrade]['tradeHour'])) {
				...
				if ($this->candleIsDoji($candleNextToLast->openBid, $candleNextToLast->highBid, $candleNextToLast->lowBid, $candleNextToLast->closeBid)) {
				...
				if (count($trades->trades) == 0) {
					...
					if ($dojiBias == "down") {
						...
						if ($range > $this->settings[$pairToTrade]['minSL']) {
